...
<div id="container">
    ...
    <div id="main">
        <div class="row">
            ...
            <input type="button" class="btn" id="find" value="查询是否可用">
        </div>
        <div class="row">
            <p>密码： </p>
...
<script type="text/javascript">
    ...
    $('#submit').click(function(){
        ...
        if(!(email.indexOf("@") > 1 && email.indexOf(".") > 1 && email.indexOf("@")+1 < email.indexOf("."))) {
            $('#header').text("email格式错误");
            return;
        }
        $.getJSON('register', {user_name: userName, password: password, email: email}, function(result) {
            if(result.state == 1) {
                $('#header').text("用户名不正确或者已被使用");
            }
            else {
                $('#header').text("登陆成功");
    ...
    $('#find').click(function(){
        var userName = $('#user_name').val();
        $.getJSON('try', {user_name: userName}, function(result) {
            if(result.state == 1) {
                $('#header').text("用户名不正确或者已被使用");
            }
            else {
